...
class TrainingClass(models.Model):
    ...
    price_per_student = fields.Integer(
        string="Học phí/sinh viên", default='1000000')
    total_revenue = fields.Integer(
        string="Doanh thu 1 lớp", compute='_generate_revenue', store=True)

    # Relationships
    subject_id = fields.Many2one(
        'training.subject', string="Môn học", required=True)
    teacher_id = fields.Many2one('training.teacher', string="Giảng viên")
    student_ids = fields.Many2many(
        'training.student', string="Danh sách sinh viên")
    session_ids = fields.One2many(
        'training.session', 'class_id', string="Lịch học chi tiết")

    @api.depends('price_per_student', 'student_ids')
    def _generate_revenue(self):
        for record in self:
            record.total_revenue = record.price_per_student * \
    ...
    def _onchange_description(self):
        for record in self:
            if record.name & record.start_date:
                record.description = f"Học lớp {record.name} bắt đầu từ ngày {record.start_date}"
